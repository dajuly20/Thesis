...
Um die Steuerung mit der Außenwelt zu Verbinden wurde ein WebSocket (siehe \ref{chp:grdlgn:webSocket}) Server in das Backend integriert. Wenn ein Client eine Verbindung aufgebaut hat, wird die Session in Array aufgenommen. Trennt der Client die Verbindung oder die Verbindung wird zum Beispiel aufgrund von einer Zeitüberschreitung automatisch getrennt, wird der entsprechende Client wieder aus diesem Array entfernt. Jeder Client in diesem Array bekommt ein Update, sobald aufgrund einer Änderung an einem Eingang oder aus einem anderen Grund *REF* eine Iteration ausgelöst wird. Ein schreibender Zugriff ist hierbei nur möglich, falls der Client sich vorher durch Übermittlung eines Tokens für das Schreiben auf diese Ausgangsressource authentifiziert hat.  
