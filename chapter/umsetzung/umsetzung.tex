...
In diesem Kapitel wird zunächst darauf eingegangen, wie das Logikprogramm für die Steuerung definiert wurde und wie dieses verarbeitet werden soll. Weiterhin wird erklärt wie die Verfügbaren Ein- und Ausgänge unterteilt werden und mit welchem Namens Schema sie adressiert werden. Weiterhin wird erklärt mit welcher Klassenstruktur diese Kanäle und Entitäten im Backend implementiert wurden. Nachdem dann etwas genauer auf die einzelnen Kanal und Entitätsarten eingegangen wurde, wird der Aufbau  des Programms und dessen Kern beschrieben und anhand von Quelltextausschnitten erklärt. Den Übergang zum letzten Teil bildet die Erklärung des WebSocket Servers, welcher die Schnittstelle zu der im letzten Unterabschnitt beschriebenen Benutzeroberfläche bildet. Hier wird auf die Statusanzeige, den Logik-Editor und die jeweilige Anbindung ans Backend beschrieben.
%
